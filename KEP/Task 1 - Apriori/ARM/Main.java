...
public class Main{
    ...
    static void publishAR(List<List<Integer>> itemSet){
        ot.println("Association rules are");
        for(List<Integer> item : itemSet){
            generateSubsetsAndPublishAR(item, 0, new ArrayList<>(), new ArrayList<>());
        }
    }
    static double confidence(List<Integer> X, List<Integer> Y){
        if(X.size()==0 || Y.size() == 0) return 0;
        List<Integer> tempX = new ArrayList<>(X);
        for(int x:Y) tempX.add(x);
        Collections.sort(tempX);
        double sup1 = support(tempX);
        // ot.print(tempX+" "+sup1+"\t");
        double sup2 = support(X);
        // ot.print(X+" "+sup1);
        // ot.println();
        return sup1/sup2;
    }
    static void printAR(List<Integer> X, List<Integer> Y){
        ot.print("(");
        for(int x : X)
            ot.print(x+" ");
        ot.print(") ----> (");
    ...
    static void generateSubsetsAndPublishAR(List<Integer> item, int ind, List<Integer> X, List<Integer> Y){
        if(ind == item.size()){
            if(confidence(X, Y) >= 0.5){
                printAR(X, Y);
            }
            return;
        }
        // Option 1
        X.add(item.get(ind));
        generateSubsetsAndPublishAR(item, ind+1, X, Y);
        X.remove(X.size()-1);
    ...
    static List<List<Integer>> lastC;
}
